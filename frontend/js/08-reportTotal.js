...
const API_BASE = "https://hoofed-shantell-superaffluently.ngrok-free.dev"; // FastAPI 주소
...
async function loadFromDB(range = "7d") {
    ...
    const userId = urlParams.get("user_id") || localStorage.getItem("user_id") || "admin";
    const r = (range || "7d").toLowerCase();
    
    // endpoint 설정
    const endpoint = `/api/report/analysis/user/${encodeURIComponent(userId)}?range=${encodeURIComponent(r)}`;

    try {
        // apiCall을 호출할 때 두 번째 인자로 헤더를 넘겨줍니다.
        const data = await apiCall(endpoint, {
            method: "GET",
            headers: {
                "ngrok-skip-browser-warning": "69420" // 👈 여기서 한 번 더 확실하게 넣어줍니다!
            }
        });
...
// 1. 리포트 생성 API 호출 함수 (주소 변경됨: /post/{post_idx} -> /post/user/{user_id})
async function generateLLMReportByPostIdx(ignore, lang = "ko") {
  // 이제 post_idx를 쓰지 않고 user_id를 가져옵니다.
  const urlParams = new URLSearchParams(window.location.search);
  const userId = urlParams.get("user_id") || localStorage.getItem("user_id") || "admin";
  const r = (__RANGE_FILTER__ || "7d");
  
  // ⭐ [중요] 백엔드 라우터에서 설정한 user_id 기반 경로로 수정
  const url = `${API_BASE}/api/report/post/user/${encodeURIComponent(userId)}?lang=${encodeURIComponent(lang)}&range=${encodeURIComponent(r)}`;
  
  console.log("[LLM 요청 전송]", url);
...
function wireLLMGenerateButton(){
  ...
  btn.addEventListener("click", async ()=>{
    try{
      ...
      btn.textContent = "AI 코치가 분석 중...";
      
      const urlParams = new URLSearchParams(window.location.search);
      const userId = urlParams.get("user_id") || localStorage.getItem("user_id") || "admin";
      
      console.log("[LLM GENERATE START] user_id=", userId);
      
      // 1. API 호출로 AI 답변 데이터 먼저 받아오기
      const report = await generateLLMReportByPostIdx(userId, "ko");

      // 2. [중요] 차트와 카드 전체 레이아웃을 먼저 갱신합니다.
      // 이 함수가 실행되면 화면이 한 번 초기화(Reset)됩니다.
      await refreshByRange(__RANGE_FILTER__);

      // 3. [중요] 모든 화면 갱신이 끝난 '직후'에 AI 답변을 화면에 그립니다.
      // 그래야 초기화된 카드 위에 AI 피드백이 고정됩니다.
      renderLLMReport(report);

      btn.textContent = "분석 리포트 업데이트 완료";
