...
OUTPUT_VIDEO = "/Users/minji/Documents/minton-angle/backend/data/standard/analyzed_swing_10"
IMAGE_SAVE_DIR = "/Users/minji/Documents/minton-angle/backend/data/standard/keyframes_images_10"
...
def analyze_badminton_swing(video_path):
    ...
    # -----------------------------------------------------------------
    # 1. IMPACT (수정됨): 손목 최고점 ~ 손목이 팔꿈치보다 높은(위) 구간 내 탐색
    # -----------------------------------------------------------------
    # A. 손목 최고점(y 최소값) 찾기
    highest_wrist_frame = min(valid_frames, key=lambda x: x['wrist'][1])
    highest_idx = highest_wrist_frame['idx']
    
    # B. 구간 설정: 최고점 시점부터 '손목이 팔꿈치보다 위에 있는' 구간 탐색
    impact_candidates = []
    for f in valid_frames:
        if f['idx'] >= highest_idx:
            # 손목 y가 팔꿈치 y보다 작아야 '위'에 있는 것 (화면 좌표계 기준)
            if f['wrist'][1] < f['elbow'][1]:
                impact_candidates.append(f)
            else:
                # 손목이 팔꿈치보다 같거나 아래로 내려가는 순간 탐색 종료
                if impact_candidates: break

    # C. 160.5도와 가장 근사한 프레임 추출
    target_angle = 160.5
    if impact_candidates:
        ...
        i_idx = highest_idx # 조건 만족 구간이 없으면 최고점 사용
    ...
    # -----------------------------------------------------------------
    # 3. READY (유지): 백스윙 이전 중 손목 움직임이 가장 적을 때
    # -----------------------------------------------------------------
    ready_candidates = []
...
out.release()
print(f"\n작업 완료! {IMAGE_SAVE_DIR} 폴더를 확인하세요.")