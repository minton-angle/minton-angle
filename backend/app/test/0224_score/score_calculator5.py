...
OUTPUT_DIR = '/Users/minji/Documents/minton-angle/backend/data/standard/0224_resources/wooil/wooil_score_result_5'
...
class GolfAnalyzer:
    ...
    # --------------------------------------------------------------------------
    # 2. Swing Sequence 분석 및 6장 이미지 추출
    # --------------------------------------------------------------------------
    def analyze_swing_sequence(self):
        ...
        # --- [결과물] Ready ~ Backswing 사이 균등한 6장 이미지 추출 ---
        if ready_f != -1 and back_f != -1 and back_f >= ready_f:
            # np.linspace를 통해 ready_f부터 back_f까지 정확히 6등분한 프레임 번호를 가져옵니다.
            seq_frames = np.linspace(ready_f, back_f, num=6, dtype=int)
            
            for idx, f_num in enumerate(seq_frames):
                frame, lms = self._get_frame_and_landmarks(f_num)
                if frame is not None:
                    if lms:
                        # 팔과 어깨 선을 가볍게 그려서 스윙 궤적의 변화를 잘 보이게 시각화합니다
                        self.draw_line_and_points(frame, lms, ['r_sh', 'r_el', 'r_wr'], color=(0, 255, 255), thickness=2)
