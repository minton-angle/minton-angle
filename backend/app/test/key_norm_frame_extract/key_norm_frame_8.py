...
def generate_keyframe_images_full(df, results_dict, output_dir, base_filename, img_size=(400, 600), padding=0.15):
    ...
    # [중요] 발목-뒤꿈치-발끝을 연결하여 완벽한 발 모양 형성
...
def analyze_swing_keyframes(input_csv_path, output_csv_path=None, output_img_dir=None):
    ...
    # [준거점 1] Ready/Backswing용: 손목이 가장 위에 있을 때 (y가 0에 가까울 때)
    highest_idx = df[cols['wri'][1]].idxmin()

    # 1. READY: 최고점 이전 중 정지 상태 (알고리즘 유지)
    r_idx = df.iloc[:highest_idx]['wrist_move'].idxmin() if highest_idx > 0 else 0

    # 2. BACKSWING: Ready와 최고점 사이 (알고리즘 유지)
    ...
    # [준거점 2] Impact용: 손목이 바닥(y=1)에 가장 가까운 시점
    lowest_idx = df[cols['wri'][1]].idxmax()

    # 3. IMPACT: 반드시 최저점(lowest_idx) 이후에서만 추출 (이 부분만 최적화)
    after_lowest_df = df.iloc[lowest_idx : lowest_idx + 60].copy() # 최저점 이후 60프레임 탐색
    
    if not after_lowest_df.empty:
        # [Local Smoothing] Impact 판별용 로컬 노이즈 제거
        for part in ['right_wrist', 'right_elbow', 'right_pinky']:
            after_lowest_df[f'{part}_x'] = after_lowest_df[f'{part}_x'].rolling(window=5, center=True, min_periods=1).median()
            after_lowest_df[f'{part}_y'] = after_lowest_df[f'{part}_y'].rolling(window=5, center=True, min_periods=1).median()
        
        # 스무딩된 좌표로 각도 계산
        after_lowest_df['temp_angle'] = after_lowest_df.apply(
            lambda r: calculate_angle(r[cols['elb'][0]], r[cols['elb'][1]],
                                      r[cols['wri'][0]], r[cols['wri'][1]],
                                      r[cols['hand'][0]], r[cols['hand'][1]]), axis=1)

        # 손목이 팔꿈치보다 위(y+0.05 여유)에 있는 지점 중 160.5도에 가장 가까운 지점
        impact_window = after_lowest_df[after_lowest_df[cols['wri'][1]] <= after_lowest_df[cols['elb'][1]] + 0.05].copy()
        
        if not impact_window.empty:
            i_idx = abs(impact_window['temp_angle'] - 160.5).idxmin()
        else:
            i_idx = after_lowest_df['temp_angle'].idxmax()
    else:
        i_idx = lowest_idx

    result = {'ready': int(r_idx), 'backswing': int(b_idx), 'impact': int(i_idx)}
    print(f"분석 완료 (y=1 바닥 기준 적용): {result}")
...
if __name__ == "__main__":
    ...
    RESULT_CSV = "/Users/minji/Documents/minton-angle/backend/data/standard/GT4_norm_keyframes_6.csv"
    IMAGE_DIR = "/Users/minji/Documents/minton-angle/backend/data/standard/GT4_norm_visualized_frames_6"
