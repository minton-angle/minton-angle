...
OUTPUT_VIDEO = "/Users/minji/Documents/minton-angle/backend/data/standard/analyzed_swing_Baek_4.mp4"
IMAGE_SAVE_DIR = "/Users/minji/Documents/minton-angle/backend/data/standard/keyframes_images_Baek_4"
...
def analyze_badminton_swing(video_path):
    ...
    # -----------------------------------------------------------------
    # 2. BACKSWING (수정됨): READY 이후 ~ 최고점 사이
    #    '손목 & 팔꿈치가 모두 코보다 왼쪽' + '코, 팔꿈치, 손목 거리 합 최소'
    # -----------------------------------------------------------------
    bs_candidates = []
    for f in valid_frames:
        if r_idx < f['idx'] <= highest_idx:
            # [조건 수정] 손목뿐만 아니라 팔꿈치도 코보다 뒤(왼쪽)에 있어야 함
            if f['wrist'][0] < f['nose'][0] and f['elbow'][0] < f['nose'][0]:
    ...
    if bs_candidates:
        # 조건을 만족하는 후보 중 세 점이 가장 밀집된 순간 선택
        b_idx = min(bs_candidates, key=lambda x: x[1])[0]
    else:
        # 조건을 만족하는 구간이 없으면 기존 방식(READY와 최고점 중간) 사용
        b_idx = (r_idx + highest_idx) // 2

    # -----------------------------------------------------------------
    # 3. IMPACT: 최고점 이후 구간 내 160.5도 근사
    # -----------------------------------------------------------------
