...
OUTPUT_VIDEO = "/Users/minji/Documents/minton-angle/backend/data/standard/analyzed_swing_5.mp4"
IMAGE_SAVE_DIR = "/Users/minji/Documents/minton-angle/backend/data/standard/keyframes_images_5"
...
def analyze_badminton_swing(video_path):
    ...
    valid_frames = [f for f in frames if 'wrist' in f and 'elbow' in f]
    
    # 1. IMPACT (기존 유지): 손목 y값이 최소(가장 높음)일 때
    impact_frame_data = min(valid_frames, key=lambda x: x['wrist'][1])
    i_idx = impact_frame_data['idx']

    # 2. BACKSWING (수정됨): 손목과 팔꿈치의 x좌표가 가장 비슷해지는 시점
    # 논리: |wrist_x - elbow_x| 가 최소인 프레임을 찾음
